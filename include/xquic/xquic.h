...
#define XQC_SERVER_AR_EXTRA_LOG_FILE "/home/qnwang/worknew/AR/AR.slog"
     ...
     * @param container saving the infos reading from file
     * @param file param file name
     */
    typedef xqc_int_t (*xqc_load_CCA_param_pt)(xqc_CCA_info_container_t *container, const char *file);

    /**
     * @brief added by jndu for CCA switching. save file of CCA infos
     * @param container saving the infos reading from file
     * @param file param file name
     */
    typedef xqc_int_t (*xqc_save_CCA_param_pt)(xqc_CCA_info_container_t *container, const char *file);

    /**
     * @brief added by jndu for CCA switching. get mp_map index
     * @return CCA_info index in mp_map
     */
    typedef xqc_int_t (*xqc_get_mp_map_index)(uint64_t path_id);

    /**
     * @brief added by jndu for CCA switching. get mp_map by local_addr and peer_addr
     * @return if found, return mp_map; if not, return NULL
     */
    typedef void *(*xqc_get_mp_map)(xqc_CCA_info_container_t *container, const struct sockaddr *sa_peer, socklen_t peer_addrlen);

    /**
     * @brief added by jndu for CCA switching. get metric by using user define method
     * @return float
     */
    typedef float (*xqc_get_CCA_info_metric_cb)(xqc_stream_CCA_info_t *CCA_sample, xqc_ip_CCA_info_t *CCA_info, int i);
...
#define XQC_SOCKET_EAGAIN -2
    ...
    typedef struct xqc_congestion_control_callback_s
    {
        ...
        void (*xqc_cong_ctl_set_pacing_rate)(void *cong_ctl, uint32_t pacing_rate);

        /* added by jndu for CCA Switching */
        void (*xqc_cong_ctl_set_cwnd)(void *cong_ctl, uint64_t cwnd);

        /* added by jndu for CCA switching */
        void (*xqc_cong_ctl_print_status)(void *cong_ctl, xqc_sample_t *sampler);
...
/* modified by jndu */
    ...
    XQC_EXPORT_PUBLIC_API extern xqc_cong_ctrl_callback_t xqc_reno_cb;
#endif
#ifdef XQC_ENABLE_BBR2
    XQC_EXPORT_PUBLIC_API extern xqc_cong_ctrl_callback_t xqc_bbr2_cb;
#endif
    XQC_EXPORT_PUBLIC_API extern xqc_cong_ctrl_callback_t xqc_bbr_cb;
    XQC_EXPORT_PUBLIC_API extern xqc_cong_ctrl_callback_t xqc_cubic_cb;
    XQC_EXPORT_PUBLIC_API extern xqc_cong_ctrl_callback_t xqc_unlimited_cc_cb;
    XQC_EXPORT_PUBLIC_API extern xqc_cong_ctrl_callback_t xqc_copa_cb;
    ...
    typedef struct xqc_config_s
    {
        ...
        /**
         * @brief Congestion Switching added by jndu, file location
         */
        const char *ipconf_file_name;

        /**
         * @brief Congestion Switching added by jndu, container type
         */
        xqc_CCA_info_container_type_t container_type;
    } xqc_config_t;
    ...
    typedef struct xqc_engine_callback_s
    {
        ...
        /* added by jndu for CCA Switching. Callback for read file of CCA infos */
        xqc_load_CCA_param_pt load_CCA_cb;

        /* added by jndu for CCA Switching. Callback for save file of CCA infos */
        xqc_save_CCA_param_pt save_CCA_cb;

        /* added by jndu for CCA Switching. Callback for find mp_map index */
        xqc_get_mp_map_index get_mp_index_cb;

        /* added by jndu for CCA Switching. Callback for get mp_map */
        xqc_get_mp_map get_mp_cb;
    ...
    typedef struct xqc_ip_CCA_info_s
    {
        int CCA_cnt;
        float CCA_power1;
        float CCA_power2;
    ...
    typedef enum xqc_switch_state_s
    {
        XQC_COPA,
        XQC_BBR,
#ifndef XQC_DISABLE_RENO
    ...
    typedef struct xqc_conn_settings_s
    {
        ...
        /* added by jndu for CCA switching */
        xqc_get_CCA_info_metric_cb metric_cb;
        uint8_t enable_CCA_switching;
        
        /* added by qnwang for AR millisecond 1ms=0.001s=1000us*/
...
#define XQC_CONN_INFO_LEN 400
    ...
    XQC_EXPORT_PUBLIC_API
    void *xqc_CCA_info_container_find(xqc_CCA_info_container_t *container, char *addr, size_t len);
    ...
    XQC_EXPORT_PUBLIC_API
    void xqc_CCA_info_container_free_value(xqc_ip_CCA_info_t **mp_map);
