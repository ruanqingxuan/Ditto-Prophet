...
float xqc_CCA_switching_get_metric(xqc_stream_CCA_info_t *CCA_sampler, xqc_ip_CCA_info_t *CCA_info, int index)
{
    ...
    // }
    // get metric
    return 0 * power5 + 0.3 * power2 + 0.3 * power3 + 0.3 * power4;
}
/**
...
int main(int argc, char *argv[])
{
    ...
    int ch = 0;
    while ((ch = getopt_long(argc, argv, "a:p:ec:LCs:w:r:l:u:x:6bS:MR:o:K:EmQ:U:yHY:k:", long_opts, NULL)) != -1)
    {
        switch (ch)
        {
        case 'Y':
    ...
    xqc_conn_settings_t conn_settings = {
        ...
        .enable_CCA_switching = g_enable_CCA_switching,
        // added by qnwang for AR
        .expected_time = (xqc_msec_t)g_expected_time,
    };
