...
{
    default_conn_settings.enable_CCA_switching = settings->enable_CCA_switching;
    default_conn_settings.metric_cb = settings->metric_cb;
    default_conn_settings.init_cong_ctrl = settings->init_cong_ctrl;
    // added by qnwang for AR
    default_conn_settings.expected_time = settings->expected_time;
...
xqc_conn_init_trans_settings(xqc_connection_t *conn)
{
    ...
    ls->max_data = ls->max_streams_bidi * ls->max_stream_data_bidi_local + ls->max_streams_uni * ls->max_stream_data_uni;
                ...
                const xqc_conn_settings_t *settings, void *user_data, xqc_conn_type_t type, xqc_ip_CCA_info_t **mp_map)
{
    ...
    if (type == XQC_CONN_TYPE_CLIENT && !xqc_check_proto_version_valid(settings->proto_version))
    {
    ...
    // qnwang set config from test_server
    if (xc->conn_settings.expected_time == 0)
    {
        xc->conn_settings.expected_time = default_conn_settings.expected_time; // default_conn_settings.expected_time
    }
    xqc_conn_init_trans_settings(xc);
    xqc_conn_init_flow_ctl(xc);
    xqc_conn_init_key_update_ctx(xc);
    // added by qnwang for AR_init
    xc->active_retrans = xqc_active_retrans_init();
    // added by qnwang for re_pkt
    xc->redundant_pkt = xqc_redundant_pkt_init();
    ...
    // add for extra_log
    xc->CS_extra_log = engine->CS_extra_log;
    xc->AR_extra_log = engine->AR_extra_log;
...
xqc_conn_encode_local_tp(xqc_connection_t *conn, uint8_t *dst, size_t dst_cap, size_t *dst_len)
{
    ...
    xqc_transport_params_type_t tp_type =
        (conn->conn_type == XQC_CONN_TYPE_CLIENT ? XQC_TP_TYPE_CLIENT_HELLO : XQC_TP_TYPE_ENCRYPTED_EXTENSIONS);
                       ...
                       xqc_cid_t *dcid, xqc_cid_t *scid, xqc_conn_settings_t *settings, void *user_data)
{
    ...
    conn = xqc_conn_create(engine, dcid, &new_scid, settings, user_data, XQC_CONN_TYPE_SERVER, mp_map);
    if (conn == NULL)
    {
...
void xqc_conn_destroy(xqc_connection_t *xc)
{
    ...
    // added by qnwang for destroy reduntant pkt
    xqc_destroy_reduntant_pkt(xc->redundant_pkt);
}
...
void xqc_conn_set_alp_user_data(xqc_connection_t *conn, void *user_data)
{
...
void xqc_conn_schedule_packets(xqc_connection_t *conn, xqc_list_head_t *head,
                               xqc_bool_t packets_are_limited_by_cc, xqc_send_type_t send_type)
{
    ...
    xqc_list_for_each_safe(pos, next, head)
    {
        ...
        {
            path = conn->scheduler_callback->xqc_scheduler_get_path(conn->scheduler,
                                                                    conn, packet_out,
...
void xqc_on_packets_send_burst(xqc_connection_t *conn, xqc_path_ctx_t *path, ssize_t sent, xqc_usec_t now, xqc_send_type_t send_type)
{
    xqc_list_head_t *pos, *next;
    xqc_packet_out_t *packet_out;
    // 该变量记录从path缓冲区应当移除的包数量，即此次发包的数量
    ...
    xqc_list_for_each_safe(pos, next, &path->path_schedule_buf[send_type])
    {
        ...
        if (xqc_has_packet_number(&packet_out->po_pkt))
        {
            ...
            if (low == -1)
            {
                low = packet_out->po_pkt.pkt_num;
            }
            /* count packets with pkt_num in the send control */
            if (XQC_CAN_IN_FLIGHT(packet_out->po_frame_types && xqc_pacing_is_on(&send_ctl->ctl_pacing)))
            {
            ...
            // added by qnwang for re_pkt
            xqc_get_packet_out(conn, packet_out);
            xqc_log(conn->log, XQC_LOG_INFO,
    ...
    if (low != -1)
    {
        xqc_extra_sample_loss_list_add(send_ctl->loss_list, low + send_count - 1, low);
    }
}

void xqc_convert_pkt_0rtt_2_1rtt(xqc_connection_t *conn, xqc_packet_out_t *packet_out)
{
                            ...
                            int congest, xqc_send_type_t send_type)
{
    ...
    xqc_list_for_each_safe(pos, next, &path->path_schedule_buf[send_type])
    {
        ...
        if (xqc_has_packet_number(&packet_out->po_pkt))
        {
            ...
            /* check cc limit */
            if (congest && !xqc_send_packet_check_cc(send_ctl, packet_out, total_bytes_to_send))
            {
                break;
            }

            /* retransmit 0-RTT packets in 1-RTT if 1-RTT keys are ready. */
            if (XQC_UNLIKELY(packet_out->po_pkt.pkt_type == XQC_PTYPE_0RTT && conn->conn_flag & XQC_CONN_FLAG_CAN_SEND_1RTT))
            {
    ...
    {
        for (int i = 1; i < conn->active_retrans->active_num; i++)
        {
            ret = xqc_send_burst(conn, path, iov_array, burst_cnt);
            if (ret < 0)
            {
                xqc_extra_log(conn->log, conn->AR_extra_log, "[active retrans ret:%ui]", ret);
            }
            else if (ret != burst_cnt)
            {
                xqc_extra_log(conn->log, conn->AR_extra_log, "[active retrans error send msg]");
                xqc_log(conn->log, XQC_LOG_ERROR, "|error send msg|sent:%ui||cnt:%d|", ret, burst_cnt);
            }
            xqc_extra_log(conn->log, conn->AR_extra_log, "[send_burst i:%ui]", i);
        }
        xqc_log(conn->log, XQC_LOG_INFO, "|Active Retransmission|send_burst active_num %ui|\n", conn->active_retrans->active_num);
    }
    
    /* burst send packets */
    // 返回值为当前发送的字节数，如果TEST_DROP为真，则返回已经发送的包数
    xqc_usec_t start = xqc_monotonic_timestamp();
    ret = xqc_send_burst(conn, path, iov_array, burst_cnt);
    xqc_usec_t end = xqc_monotonic_timestamp();
    ...
    /**
     * update power1 added by jndu
     */
    float throughput = xqc_extra_sample_throughput(ret, end - start);
    float max_throughput = xqc_max(send_ctl->ctl_max_throughput, throughput);
    send_ctl->ctl_throughput = throughput;
    send_ctl->ctl_max_throughput = max_throughput;
    xqc_log(conn->log, XQC_LOG_DEBUG, "|CCA switching|throughput:%.4f|max_throughput:%.4f|",
...
void xqc_path_send_packets_batch(xqc_connection_t *conn, xqc_path_ctx_t *path,
                                 xqc_list_head_t *head, int congest, xqc_send_type_t send_type)
{
    // xqc_log(conn->log, XQC_LOG_INFO, "|jndu_test|send_packets_batch|");
                           ...
                           xqc_list_head_t *head, int congest, xqc_send_type_t send_type)
{
    ...
    xqc_list_for_each_safe(pos, next, &path->path_schedule_buf[send_type])
    {
        ...
        /* check cc limit */
        if (congest && !xqc_send_packet_check_cc(send_ctl, packet_out, 0))
        {
            break;
        }

        start = xqc_monotonic_timestamp();
        ret = xqc_path_send_one_packet(conn, path, packet_out);
        end = xqc_monotonic_timestamp();
...

        {
            ...
            size = packet_out->po_used_size;
            throughput += ((float)size / (float)(end - start));
            cnt++;
        }
    ...
    /* sample and CCA switching */
    if (low != -1)
    {
        xqc_extra_sample_loss_list_add(send_ctl->loss_list, low + cnt - 1, low);
        xqc_send_ctl_t *send_ctl = path->path_send_ctl;
...
xqc_need_padding(xqc_connection_t *conn, xqc_packet_out_t *packet_out)
{
    ...
    else if ((packet_out->po_frame_types & XQC_FRAME_BIT_PATH_CHALLENGE) || (packet_out->po_frame_types & XQC_FRAME_BIT_PATH_RESPONSE) || (packet_out->po_flag & XQC_POF_PMTUD_PROBING))
    {
        return XQC_TRUE;
    }
...
xqc_send_packet_with_pn(xqc_connection_t *conn, xqc_path_ctx_t *path, xqc_packet_out_t *packet_out)
{
    ...
    if (conn->active_retrans->flag == XQC_TRUE)
    {
        for (int i = 1; i < conn->active_retrans->active_num; i++)
        {
            ...
            if (sent != conn->enc_pkt_len)
            {
                xqc_extra_log(conn->log, conn->AR_extra_log, "[active retrans:write_socket error]");
                xqc_log(conn->log, XQC_LOG_ERROR, "|write_socket error|");
            }
            else
            {
                xqc_extra_log(conn->log, conn->AR_extra_log, "[send_pn i:%d] [pkt_num:%ui]", i, packet_out->po_pkt.pkt_num);
                xqc_log(conn->log, XQC_LOG_INFO, "|<==|conn:%p|path:%ui|", conn, path->path_id);
            }
        }
        xqc_extra_log(conn->log, conn->AR_extra_log, "[send_pn num:%ui] [pkt_num:%ui] [eng_type:%s]",
                      conn->active_retrans->active_num, packet_out->po_pkt.pkt_num, conn_type[conn->conn_type]);
        xqc_log(conn->log, XQC_LOG_INFO, "|Active Retransmission|send_pn active_num %ui|\n", conn->active_retrans->active_num);
    }
...
void xqc_conn_check_path_utilization(xqc_connection_t *conn)
{
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list)
    {
        ...
        if (!xqc_path_is_full(path) && conn->scheduler_callback->xqc_scheduler_handle_path_event)
        {
...
void xqc_path_send_one_or_two_ack_elicit_pkts(xqc_path_ctx_t *path, xqc_pkt_num_space_t pns)
{
    ...
    while (probe_num > 0)
    {
        xqc_list_for_each_safe(pos, next, &c->conn_send_queue->sndq_unacked_packets[pns])
        {
            ...
            if (XQC_IS_ACK_ELICITING(packet_out->po_frame_types) && (XQC_NEED_REPAIR(packet_out->po_frame_types) || (packet_out->po_frame_types & XQC_FRAME_BIT_DATAGRAM && c->conn_settings.datagram_force_retrans_on)))
            {
...
void xqc_conn_shutdown(xqc_connection_t *conn)
{
    xqc_path_ctx_t *path;
    xqc_list_head_t *pos, *next;
...
xqc_conn_immediate_close(xqc_connection_t *conn)
{
    ...
    if (!(conn->conn_flag & XQC_CONN_FLAG_INIT_RECVD) && conn->conn_type == XQC_CONN_TYPE_SERVER)
    {
        conn->conn_state = XQC_CONN_STATE_CLOSED;
...
void xqc_conn_info_print(xqc_connection_t *conn, xqc_conn_stats_t *conn_stats)
{
...
void xqc_conn_get_stats_internal(xqc_connection_t *conn, xqc_conn_stats_t *conn_stats)
{
    /* 1. 与路径无关的连接级别埋点 */
    ...
    if (conn->conn_initial_path && conn->conn_initial_path->path_send_ctl && conn->conn_initial_path->path_pn_ctl)
    {
...
xqc_conn_early_data_reject(xqc_connection_t *conn)
{
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_all_streams)
    {
        ...
        if (stream->stream_flag & XQC_STREAM_FLAG_HAS_0RTT)
        {
            ...
            if (stream->stream_state_send >= XQC_SEND_STREAM_ST_RESET_SENT || stream->stream_state_recv >= XQC_RECV_STREAM_ST_RESET_RECVD)
            {
...
xqc_conn_handshake_complete(xqc_connection_t *conn)
{
    ...
    if (conn->conn_type == XQC_CONN_TYPE_SERVER)
    {
        ...
        if (!(conn->conn_flag & XQC_CONN_FLAG_TOKEN_OK) || conn->conn_flag & XQC_CONN_FLAG_UPDATE_NEW_TOKEN)
        {
    ...
    if ((conn->conn_flag & XQC_CONN_FLAG_HANDSHAKE_COMPLETED) && ((conn->conn_type == XQC_CONN_TYPE_CLIENT && conn->conn_flag & XQC_CONN_FLAG_HAS_0RTT) || conn->conn_type == XQC_CONN_TYPE_SERVER) && !(conn->conn_flag & XQC_CONN_FLAG_0RTT_OK) && !(conn->conn_flag & XQC_CONN_FLAG_0RTT_REJ))
    {
...
{
    if (conn->undecrypt_count[encrypt_level] >= XQC_UNDECRYPT_PACKET_MAX || packet_in->buf_size > XQC_MAX_PACKET_IN_LEN)
    {
    ...
    if (conn->conn_type == XQC_CONN_TYPE_SERVER && !(conn->conn_flag & XQC_CONN_FLAG_DCID_OK) && encrypt_level == XQC_ENC_LEV_0RTT && conn->undecrypt_count[encrypt_level] > XQC_UNDECRYPT_0RTT_MAX_BEFORE_INIT)
    {
...
char *xqc_conn_addr_str_without_cid(xqc_connection_t *conn)
{
    if (conn->local_addrlen == 0 || conn->peer_addrlen == 0)
    {
        return "addr not avail";
    }
    ...
    if (conn->addr_str_len == 0)
    {
        ...
        conn->addr_str_without_cid_len = snprintf(conn->addr_str_without_cid, sizeof(conn->addr_str_without_cid), "l-%s-%d p-%s-%d",
                                                  xqc_local_addr_str((struct sockaddr *)sa_local, conn->local_addrlen),
    ...
    return conn->addr_str_without_cid;
}

char *
xqc_path_addr_str(xqc_path_ctx_t *path)
{
    if (path->local_addrlen == 0 || path->peer_addrlen == 0 || path->path_scid.cid_len == 0 || path->path_dcid.cid_len == 0)
    {
        return "addr or cid not avail";
    }
...
void xqc_conn_record_single(xqc_connection_t *c, xqc_packet_in_t *packet_in)
{
 ...
 */
void xqc_conn_addr_validated(xqc_connection_t *c)
{
    c->conn_flag |= XQC_CONN_FLAG_ADDR_VALIDATED;
    xqc_log(c->log, XQC_LOG_INFO, "|Address Validated|conn:%p|role:%d|", c, c->conn_type);
}

void xqc_conn_server_validate_address(xqc_connection_t *c, xqc_packet_in_t *pi)
{
    ...
    case XQC_PTYPE_INIT:
        ...
        else
        {
             ...
             */
            if (c->scid_set.user_scid.cid_len >= XQC_CONN_ADDR_VALIDATION_CID_ENTROPY && xqc_cid_in_cid_set(&c->scid_set.cid_set, &c->original_dcid) == NULL && xqc_cid_in_cid_set(&c->scid_set.cid_set, &pi->pi_pkt.pkt_dcid) != NULL)
            {
                xqc_conn_addr_validated(c);
            }
        }
...
void xqc_conn_client_validate_address(xqc_connection_t *c, xqc_packet_in_t *pi)
{
    switch (pi->pi_pkt.pkt_type)
    {
    case XQC_PTYPE_INIT:
...
void xqc_conn_validate_address(xqc_connection_t *c, xqc_packet_in_t *pi)
{
...
{
    if (-XQC_EVERSION == ret || -XQC_EILLPKT == ret || -XQC_EWAITING == ret || -XQC_EIGNORE_PKT == ret || -XQC_EDECRYPT == ret)
    {
        return XQC_TRUE;
    }
...
xqc_conn_check_tx_key(xqc_connection_t *conn)
{
    ...
    if (!(conn->conn_flag & XQC_CONN_FLAG_CAN_SEND_1RTT) && xqc_tls_is_key_ready(conn->tls, XQC_ENC_LEV_1RTT, XQC_KEY_TYPE_TX_WRITE))
    {
...
xqc_conn_check_handshake_complete(xqc_connection_t *conn)
{
    ...
    if (!(conn->conn_flag & XQC_CONN_FLAG_HANDSHAKE_COMPLETED) && conn->conn_state == XQC_CONN_STATE_ESTABED)
    {
...
{
    if ((xqc_conn_is_handshake_confirmed(conn)) && (conn->scid_set.cid_set.unused_cnt == 0) && (conn->scid_set.cid_set.used_cnt < conn->remote_settings.active_connection_id_limit))
    {
...
void xqc_conn_ptmud_probing(xqc_connection_t *conn)
{
    ...
    if (!(conn->conn_flag & XQC_CONN_FLAG_CAN_SEND_1RTT))
    {
        if ((conn->conn_type == XQC_CONN_TYPE_CLIENT) && (conn->conn_state == XQC_CONN_STATE_CLIENT_INITIAL_SENT) && support_0rtt)
        {
...
xqc_conn_update_user_scid(xqc_connection_t *conn, xqc_scid_set_t *scid_set)
{
    ...
    xqc_list_for_each_safe(pos, next, &scid_set->cid_set.list_head)
    {
        ...
        if (scid->state == XQC_CID_USED && xqc_cid_is_equal(&scid_set->user_scid, &scid->cid) != XQC_OK)
        {
...
xqc_conn_peer_complete_address_validation(xqc_connection_t *c)
{
    ...
    {
        return (c->conn_flag & XQC_CONN_FLAG_HANDSHAKE_CONFIRMED) || xqc_send_ctl_ack_received_in_pns(c->conn_initial_path->path_send_ctl, XQC_PNS_HSK);
    }
}
...
{
    return xqc_tls_is_key_ready(c->tls, XQC_ENC_LEV_HSK, XQC_KEY_TYPE_TX_WRITE) && xqc_tls_is_key_ready(c->tls, XQC_ENC_LEV_HSK, XQC_KEY_TYPE_RX_READ);
}

static xqc_bool_t
xqc_need_reassemble_packet(xqc_packet_out_t *packet_out)
{
    if (packet_out->po_pkt.pkt_type == XQC_PTYPE_INIT && packet_out->po_frame_types & XQC_FRAME_BIT_CRYPTO)
    {
        return XQC_TRUE;
    }
...
xqc_conn_check_transport_params(xqc_connection_t *conn, const xqc_transport_params_t *params)
{
    ...
    if (params->initial_max_streams_bidi > XQC_MAX_STREAMS || params->initial_max_streams_uni > XQC_MAX_STREAMS || params->initial_max_stream_data_bidi_local > XQC_MAX_STREAMS || params->initial_max_stream_data_bidi_remote > XQC_MAX_STREAMS || params->initial_max_stream_data_uni > XQC_MAX_STREAMS)
    {
...
void xqc_conn_update_flow_ctl_settings(xqc_connection_t *conn)
{
    xqc_conn_flow_ctl_t *flow_ctl = &conn->conn_flow_ctl;
...
void xqc_conn_tls_keylog_cb(const char *line, void *user_data)
{
...
void xqc_conn_tls_error_cb(xqc_int_t tls_err, void *user_data)
{
    xqc_connection_t *conn = (xqc_connection_t *)user_data;
...
void xqc_free_crypto_buffer_list(xqc_list_head_t *buffer_list)
{
...
void xqc_conn_tls_handshake_completed_cb(void *user_data)
{
    xqc_connection_t *conn = (xqc_connection_t *)user_data;
...
{
    if ((packet_out->po_frame_types & XQC_FRAME_BIT_STREAM) && !(packet_out->po_flag & XQC_POF_STREAM_UNACK))
    {
        if ((!packet_out->po_origin))
        {
            xqc_stream_t *stream;
            for (int i = 0; i < XQC_MAX_STREAM_FRAME_IN_PO; i++)
            {
                ...
                if (stream != NULL)
                {
                    ...
                    if (packet_out->po_stream_frames[i].ps_has_fin && stream->stream_state_send == XQC_SEND_STREAM_ST_SEND)
                    {
...
{
    if (conn->transport_cbs.conn_closing && (conn->conn_flag & XQC_CONN_FLAG_CLOSING_NOTIFY))
    {
        conn->conn_flag &= ~XQC_CONN_FLAG_CLOSING_NOTIFY;
...
int xqc_conn_buff_0rtt_datagram(xqc_connection_t *conn, void *data,
...
void xqc_conn_reset(xqc_connection_t *conn)
{
    xqc_conn_shutdown(conn);
