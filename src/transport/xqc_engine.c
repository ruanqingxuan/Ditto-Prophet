...
xqc_config_t default_client_config = {
    ...
    .sendmmsg_on = 0,
    .enable_h3_ext = 0,
    .ipconf_file_name = "ip_client.xml"};
...
xqc_config_t default_server_config = {
    ...
    .sendmmsg_on = 0,
    .enable_h3_ext = 0,
    .ipconf_file_name = "ip_server.xml"};

void xqc_engine_free_alpn_list(xqc_engine_t *engine);
...
xqc_set_config(xqc_config_t *dst, const xqc_config_t *src)
{
    ...
    dst->ipconf_file_name = src->ipconf_file_name;
...
xqc_engine_CCA_info_container_create(xqc_engine_t *engine)
{
    xqc_CCA_info_container_t *container = NULL;
    container = xqc_malloc(sizeof(xqc_CCA_info_container_t));
    /**
     * default hash type
     */
    xqc_CCA_info_container_type_t container_type = XQC_CCA_INFO_HASH;
    if (engine->config)
    {
        container_type = engine->config->container_type;
    }
    if (xqc_CCA_info_container_init(container, container_type))
    {
        xqc_free(container);
        container = NULL;
    }
    if (engine->eng_callback.load_CCA_cb)
    {
        if (engine->eng_callback.load_CCA_cb(container, engine->config->ipconf_file_name) < 0)
        {
            xqc_free(container);
            container = NULL;
        }
 ...
 * Congestion Switching added by jndu
 */
void xqc_engine_CCA_info_container_destroy(xqc_engine_t *engine)
{
    if (engine->eng_callback.save_CCA_cb)
    {
        engine->eng_callback.save_CCA_cb(engine->container, engine->config->ipconf_file_name);
    }
    xqc_CCA_info_container_release(engine->container);
}
...
void xqc_engine_conns_pq_destroy(xqc_pq_t *q)
{
    xqc_pq_destroy(q);
    xqc_free(q);
}

void xqc_engine_wakeup_pq_destroy(xqc_wakeup_pq_t *q)
{
    xqc_wakeup_pq_destroy(q);
    xqc_free(q);
}
                  ...
                  void *user_data)
{
    ...
    if (xqc_engine_check_config(engine_type, engine_config, ssl_config, transport_cbs) == XQC_FALSE)
    {
        return NULL;
    }
    ...
    if (engine_config != NULL && xqc_engine_set_config(engine, engine_config) != XQC_OK)
    {
        goto fail;
    }
    ...
    engine->container = xqc_engine_CCA_info_container_create(engine);
...
void xqc_engine_destroy(xqc_engine_t *engine)
{
    ...
    if (engine->container)
    {
        xqc_engine_CCA_info_container_destroy(engine);
...
#define XQC_CHECK_UNDECRYPT_PACKETS()                           \
    do                                                          \
    {                                                           \
...
#define XQC_CHECK_IMMEDIATE_CLOSE()                                         \
    do                                                                      \
    {                                                                       \
...
void xqc_engine_process_conn(xqc_connection_t *conn, xqc_usec_t now)
{
    ...
    if (XQC_UNLIKELY(!xqc_list_empty(&conn->conn_send_queue->sndq_buff_1rtt_packets) && conn->conn_flag & XQC_CONN_FLAG_CAN_SEND_1RTT))
    {
    ...
    /* for multi-path */
    if ((conn->conn_flag & XQC_CONN_FLAG_NEW_CID_RECEIVED) && xqc_conn_check_unused_cids(conn) == XQC_OK)
    {
                          ...
                          xqc_usec_t recv_time, void *user_data)
{
    ...
    if (XQC_UNLIKELY(conn == NULL))
    {
        ...
        if (XQC_PACKET_IS_LONG_HEADER(packet_in_buf))
        {
            ...
            if (engine->eng_type == XQC_ENGINE_SERVER && (XQC_PACKET_LONG_HEADER_GET_TYPE(packet_in_buf) == XQC_PTYPE_INIT || XQC_PACKET_LONG_HEADER_GET_TYPE(packet_in_buf) == XQC_PTYPE_0RTT) && (local_addr != NULL && peer_addr != NULL))
            {
...
process:
    ...
    /* NAT rebinding */
    if (engine->eng_type == XQC_ENGINE_SERVER && (peer_addr != NULL && peer_addrlen != 0) && !xqc_is_same_addr_as_any_path(conn, peer_addr))
    {
        xqc_path_ctx_t *path = xqc_conn_find_path_by_scid(conn, &scid);
        if ((path != NULL) && (path->path_state == XQC_PATH_STATE_ACTIVE))
        {

            if ((path->rebinding_addrlen == 0) && !xqc_timer_is_set(&path->path_send_ctl->path_timer_manager, XQC_TIMER_NAT_REBINDING))
            {
            ...
            else if ((path->rebinding_check_response == 0) && xqc_is_same_addr(peer_addr, (struct sockaddr *)path->rebinding_addr))
            {
    ...
    if (conn->conn_type == XQC_CONN_TYPE_SERVER && conn->conn_flag & XQC_CONN_FLAG_RECV_NEW_PATH)
    {
...
after_process:
    ...
    /* main logic */
    if (++conn->packet_need_process_count >= XQC_MAX_PACKET_PROCESS_BATCH || conn->conn_err != 0 || conn->conn_flag & XQC_CONN_FLAG_NEED_RUN)
    {
        xqc_engine_main_logic_internal(engine);
                         ...
                         xqc_app_proto_callbacks_t *ap_cbs)
{
    ...
    xqc_list_for_each_safe(pos, next, &engine->alpn_reg_list)
    {
        ...
        if (alpn_len == alpn_reg->alpn_len && xqc_memcmp(alpn, alpn_reg->alpn, alpn_len) == 0)
        {
...
xqc_engine_unregister_alpn(xqc_engine_t *engine, const char *alpn, size_t alpn_len)
{
    ...
    {
        alpn_reg = xqc_list_entry(pos, xqc_alpn_registration_t, head);
        if (alpn_reg && alpn_len == alpn_reg->alpn_len && xqc_memcmp(alpn, alpn_reg->alpn, alpn_len) == 0)
        {
...
{
    return engine->config->sendmmsg_on && (engine->transport_cbs.write_mmsg || engine->transport_cbs.write_mmsg_ex);
}
