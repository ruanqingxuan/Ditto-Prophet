...
{
    // 用于处理发送队列中可能需要移除未确认数据包
    // it is only called from loss detection function 仅从损耗检测功能中调用
    // po must not be inflight po po必须在未接触的列表中
    // po must be in unacked list po必须在未打入的列表中
    po->po_flag |= XQC_POF_DROPPED_DGRAM;
    if (po->po_origin == NULL)
    {
        if (po->po_origin_ref_cnt == 0)
        {
            // 将po从未确认列表中移除，并插入到连接的发送队列的空闲数据包列表中
...
int xqc_send_ctl_indirectly_ack_or_drop_po(xqc_connection_t *conn, xqc_packet_out_t *packet_out)
{
    // 间接确认或丢弃数据包
    // 根据数据包的路径ID查找路径上下文
    ...
    if (packet_out->po_acked || (packet_out->po_origin && packet_out->po_origin->po_acked))
    {
    ...
    {
        if ((packet_out->po_flag & XQC_POF_DROPPED_DGRAM) || (packet_out->po_origin && (packet_out->po_origin->po_flag & XQC_POF_DROPPED_DGRAM)))
        {
...
xqc_send_ctl_create(xqc_path_ctx_t *path)
{
    ...
    if (!conn->conn_settings.enable_CCA_switching)
    {
    ...
    /* added by jndu, metric cb init */
    send_ctl->ctl_metric_cb = conn->conn_settings.metric_cb;

    /* added by jndu, init CCA_sampler */
    memset(&send_ctl->CCA_sampler, 0,
           sizeof(send_ctl->CCA_sampler));
    return send_ctl;
}
...
void xqc_send_ctl_destroy(xqc_send_ctl_t *send_ctl)
{
    ...
    // 删除loss_list所有节点
    xqc_extra_sample_loss_list_del(send_ctl->loss_list, send_ctl->loss_list->list_head.prev);
    xqc_free(send_ctl->loss_list);
    /* added by jndu for CCA switching */
    xqc_switch_ctx_destroy(send_ctl);
    xqc_free(send_ctl->ctl_switch_ctx);
}
...
void xqc_send_ctl_info_circle_record(xqc_send_ctl_t *send_ctl)
{
    xqc_connection_t *conn = send_ctl->ctl_conn;
    ...
    if (ctl_info->record_interval < 10000)
    { /* minimum 10ms interval to avoid log flooding */
        return;
    }

    if (ctl_info->last_record_time + ctl_info->record_interval > now)
    { /* not yet time to record */
        return;
    }
    ...
    if (send_ctl->ctl_cong_callback->xqc_cong_ctl_init_bbr)
    {
        ...
        pacing_rate = send_ctl->ctl_cong_callback->xqc_cong_ctl_get_pacing_rate(send_ctl->ctl_cong);
 ...
 * QUIC的拥塞控制基于TCP NewReno [RFC6582]。 NewReno是一种基于拥塞的窗户的拥塞控制。
 * QUIC指定了由于更精细的控制和适当的字节计数[RFC3465]而导致的字节中的拥塞窗口。
 *
 ...
 * 如果QUIC主机将bytes_in_flight（定义见附录B.2）增加到可用拥塞窗口之外，
 * 则不得发送数据包，除非数据包是在PTO定时器到期后发送的探测数据包，如第6.3节所述。
 ...
 * 实现可以使用其他拥塞控制算法，例如Cubic[RFC8312]，并且端点可以使用彼此不同的算法。
 * QUIC为拥塞控制提供的信号是通用的，并且被设计为支持不同的算法。
 */
// 是否可以发送数据
...
{
    return xqc_send_packet_cwnd_allows(send_ctl, po, schedule_bytes) && xqc_send_packet_pacer_allows(send_ctl, po, schedule_bytes);
}
...
void xqc_send_queue_maybe_remove_unacked(xqc_packet_out_t *packet_out, xqc_send_queue_t *send_queue, xqc_path_ctx_t *path)
{
    ...
    if (packet_out->po_origin && (--packet_out->po_origin->po_origin_ref_cnt) == 0)
    {
...
// 处理数据包中的 RESET_STREAM 帧被确认的情况
void xqc_send_ctl_on_reset_stream_acked(xqc_send_ctl_t *send_ctl, xqc_packet_out_t *packet_out)
{
...
// 用于处理 PNS（Packet Number Space）被丢弃的情况
...
void xqc_send_ctl_on_packet_sent(xqc_send_ctl_t *send_ctl, xqc_pn_ctl_t *pn_ctl, xqc_packet_out_t *packet_out, xqc_usec_t now)
{
    ...
    // 若为阻塞包和最大包
    if (packet_out->po_frame_types & (XQC_FRAME_BIT_DATA_BLOCKED | XQC_FRAME_BIT_STREAM_DATA_BLOCKED | XQC_FRAME_BIT_MAX_STREAM_DATA | XQC_FRAME_BIT_MAX_DATA))
    {
        // 记录日志
        xqc_log(send_ctl->ctl_conn->log, XQC_LOG_DEBUG,
    ...
    if (XQC_CAN_IN_FLIGHT(packet_out->po_frame_types))
    {
        ...
        // 如果飞行中的数据包数为零，根据拥塞控制算法进行处理
        if (send_ctl->ctl_bytes_in_flight == 0)
        {
            // 拥塞用的是bbr
            if (send_ctl->ctl_cong_callback->xqc_cong_ctl_init_bbr && send_ctl->ctl_app_limited > 0)
            {
                uint8_t mode, idle_restart;
                mode = send_ctl->ctl_cong_callback->xqc_cong_ctl_info_cb->mode(send_ctl->ctl_cong);
                idle_restart = send_ctl->ctl_cong_callback->xqc_cong_ctl_info_cb->idle_restart(send_ctl->ctl_cong);
                xqc_log(send_ctl->ctl_conn->log, XQC_LOG_DEBUG,
                        "|BeforeRestartFromIdle|mode %ud|idle %ud"
                        "|bw %ud|pacing rate %ud|",
                        (unsigned int)mode, (unsigned int)idle_restart, send_ctl->ctl_cong_callback->xqc_cong_ctl_get_bandwidth_estimate(send_ctl->ctl_cong),
        ...
        // 如果是激发 ACK 的数据包，设置损失检测定时器
        if (XQC_IS_ACK_ELICITING(packet_out->po_frame_types))
        {
            xqc_send_ctl_set_loss_detection_timer(send_ctl);
        }

        // 若丢包，记录丢包数，并将packet_out取消设置LOST，进行重传
        ...
        // 若为tlp（Tail Loss Probe）包，记录tlp数
        ...
        if (packet_out->po_frame_types & XQC_FRAME_BIT_DATAGRAM)
        {
            ...
            if (packet_out->po_flag & XQC_POF_QOS_HIGH)
            {
                ...
                if (packet_out->po_flag & XQC_POF_REINJECTED_REPLICA)
                {
                    ...
                    if (packet_out->po_origin && packet_out->po_path_id != packet_out->po_origin->po_path_id)
                    {
                        send_ctl->ctl_conn->dgram_stats.hp_red_dgram_mp++;
                    }
                }
            }
            // 如果数据包标记为 QoS 探测，增加相应计数
...
int xqc_send_ctl_on_ack_received(xqc_send_ctl_t *send_ctl, xqc_pn_ctl_t *pn_ctl, xqc_send_queue_t *send_queue, xqc_ack_info_t *const ack_info, xqc_usec_t ack_recv_time, xqc_bool_t ack_on_same_path)
{
    ...
    /* detect and remove acked packets 探测并移除已确认的包 */
    xqc_list_for_each_safe(pos, next, &send_queue->sndq_unacked_packets[pns])
    {
        packet_out = xqc_list_entry(pos, xqc_packet_out_t, po_list);
        ...
        if (packet_out->po_pkt.pkt_num >= range->low)
        {
            ...
            /* Packet previously declared lost gets acked 先前声明丢失的数据包得到确认 */
     ...
     * 重置pto计数器，除非客户端不确定服务器是否已验证客户端的地址
     */
    ...
    /* Clear app-limited field if the bubble is gone. 如果bubble消失，清除应用程序限制字段*/
    ...
    if (send_ctl->ctl_cong_callback->xqc_cong_ctl_init_bbr /* && stream_frame_acked */)
    {
        ...
        {
            if ((send_ctl->ctl_cong_callback->xqc_cong_ctl_get_bandwidth_estimate != NULL) && send_ctl->ctl_conn->log->log_level >= XQC_LOG_STATS && (send_ctl->ctl_info.last_bw_time + send_ctl->ctl_info.record_interval <= now))
            {
    ...
    {
        xqc_generate_sample(&send_ctl->sampler, send_ctl, ack_recv_time);
    }

    if (conn->conn_settings.enable_CCA_switching)
    {
        xqc_cong_ctrl_callback_t *future_cong_ctrl = xqc_switch_get_future_cc_cb(send_ctl);
        if (future_cong_ctrl)
        {
            void *future_cong = send_ctl->ctl_cong_stack[send_ctl->ctl_switch_ctx->ctx_future_state];
            future_cong_ctrl->xqc_cong_ctl_print_status(future_cong, &send_ctl->sampler);
            /* BBR 处理bbr拥塞 */
            if (future_cong_ctrl->xqc_cong_ctl_init_bbr /* && stream_frame_acked */)
            {
                ...
                if (sample_type == XQC_RATE_SAMPLE_VALID)
                {
                    if ((future_cong_ctrl->xqc_cong_ctl_get_bandwidth_estimate != NULL) && send_ctl->ctl_conn->log->log_level >= XQC_LOG_STATS && (send_ctl->ctl_info.last_bw_time + send_ctl->ctl_info.record_interval <= now))
                    {
                        bw_before = future_cong_ctrl->xqc_cong_ctl_get_bandwidth_estimate(future_cong);
                        if (bw_before != 0)
                        {
                            bw_record_flag = 1;
                        }
                    ...
                    future_cong_ctrl->xqc_cong_ctl_on_ack_multiple_pkts(future_cong, &send_ctl->sampler);
                }
                ...
                if (send_ctl->ctl_conn->log->log_level >= XQC_LOG_DEBUG)
                {
                    ...
                    float pacing_gain, cwnd_gain;
                    uint64_t min_rtt, recovery_start_time;
                    xqc_bbr_info_interface_t *info = future_cong_ctrl->xqc_cong_ctl_info_cb;
                    mode = info->mode(future_cong);
                    full_bw_reached = info->full_bw_reached(future_cong);
                    recovery_mode = info->recovery_mode(future_cong);
                    round_start = info->round_start(future_cong);
                    packet_conservation = info->packet_conservation(future_cong);
                    idle_restart = info->idle_restart(future_cong);
                    pacing_gain = info->pacing_gain(future_cong);
                    cwnd_gain = info->cwnd_gain(future_cong);
                    min_rtt = info->min_rtt(future_cong);
                    recovery_start_time = info->recovery_start_time(future_cong);
                    xqc_conn_log(conn, XQC_LOG_DEBUG,
                                 "|bbr on ack|mode:%ud|pacing_rate:%ud|bw:%ud|"
                                 "cwnd:%ui|full_bw_reached:%ud|inflight:%ud|"
                                 "srtt:%ui|latest_rtt:%ui|min_rtt:%ui|applimit:%ud|"
                                 ...
                                 "idle_restart:%ud|packet_conservation:%ud|round_start:%ud|",
                                 (unsigned int)mode,
                                 future_cong_ctrl->xqc_cong_ctl_get_pacing_rate(future_cong),
                                 future_cong_ctrl->xqc_cong_ctl_get_bandwidth_estimate(future_cong),
                                 future_cong_ctrl->xqc_cong_ctl_get_cwnd(future_cong),
                                 (unsigned int)full_bw_reached, send_ctl->ctl_bytes_in_flight,
                                 send_ctl->ctl_srtt, send_ctl->ctl_latest_rtt, min_rtt,
                                 send_ctl->sampler.is_app_limited, send_ctl->ctl_lost_count,
                                 (unsigned int)recovery_mode, recovery_start_time, (unsigned int)idle_restart,
                ...
                if (bw_record_flag)
                {
                    bw_after = future_cong_ctrl->xqc_cong_ctl_get_bandwidth_estimate(future_cong);
                    if (bw_after > 0)
                    {
                        if (xqc_sub_abs(bw_after, bw_before) * 100 > (bw_before * send_ctl->ctl_info.bw_change_threshold))
                        {
                            ...
                            xqc_conn_log(conn, XQC_LOG_STATS,
                                         "|bandwidth change record|bw_before:%ui|bw_after:%ui|srtt:%ui|cwnd:%ui|",
                                         bw_before, bw_after, send_ctl->ctl_srtt, future_cong_ctrl->xqc_cong_ctl_get_cwnd(future_cong));
                        }
                    }
                }
            }
            else if (future_cong_ctrl->xqc_cong_ctl_on_ack_multiple_pkts)
            {
                // 除bbr之外其他拥塞处理
                xqc_sample_type_t sample_type = xqc_generate_sample(&send_ctl->sampler, send_ctl, ack_recv_time);
    ...
    /* log for cwnd, srtt etc. */
    uint64_t cwnd = 0, srtt = 0, pacing_rate = 0;
    if (send_ctl->ctl_cong_callback->xqc_cong_ctl_get_pacing_rate)
    {
        pacing_rate = send_ctl->ctl_cong_callback->xqc_cong_ctl_get_pacing_rate(send_ctl->ctl_cong);
    }
    if (send_ctl->ctl_cong_callback->xqc_cong_ctl_get_cwnd)
    {
        cwnd = send_ctl->ctl_cong_callback->xqc_cong_ctl_get_cwnd(send_ctl->ctl_cong);
    }

    send_ctl->ctl_cong_callback->xqc_cong_ctl_print_status(send_ctl->ctl_cong, &send_ctl->sampler);

    xqc_extra_log(conn->log, conn->CS_extra_log, "[cwnd:%ui] [srtt:%ui] [pacing rate:%ui]", cwnd, send_ctl->ctl_srtt, pacing_rate);
    xqc_log(conn->log, XQC_LOG_INFO, "[cwnd:%ui] [srtt:%ui] [pacing rate:%ui]", cwnd, send_ctl->ctl_srtt, pacing_rate);

    /* attempt to do CCA switching */
    xqc_switch_CCA(send_ctl);
...
void xqc_send_ctl_on_dgram_received(xqc_send_ctl_t *send_ctl, size_t dgram_size)
{
     ...
     * 如果此数据报解除了服务器的防放大限制，请启用PTO计时器以避免死锁
     */
...
// 跟踪最新往返时间（RTT）
void xqc_send_ctl_latest_rtt_tracking(xqc_send_ctl_t *send_ctl, xqc_usec_t *latest_rtt)
{
...
void xqc_send_ctl_update_rtt(xqc_send_ctl_t *send_ctl, xqc_usec_t *latest_rtt, xqc_usec_t ack_delay)
{
    ...
    else
    {
        ...
        /* Adjust for ack delay if it's plausible. 如果合理，则调整ack延迟 */
        xqc_usec_t adjusted_rtt = *latest_rtt;
        if (adjusted_rtt > ack_delay && (adjusted_rtt + 1000) >= (send_ctl->ctl_minrtt + ack_delay))
        {
        ...
        send_ctl->ctl_rttvar += (send_ctl->ctl_srtt > adjusted_rtt
                                     ? send_ctl->ctl_srtt - adjusted_rtt
                                     : adjusted_rtt - send_ctl->ctl_srtt) >>
                                2;
    ...
    // added by qnwang for AR
    if (send_ctl->ctl_conn->conn_settings.expected_time != 0)
    {
        xqc_update_success_probability(send_ctl->ctl_conn, *latest_rtt);
    }
                                            ...
                                            xqc_usec_t spurious_loss_sent_time)
{
    ...
    /* Adjust Packet Threshold 调整数据包阈值 */
    ...
    while (max_rtt + (max_rtt >> send_ctl->ctl_reordering_time_threshold_shift) < reorder_time_interval && send_ctl->ctl_reordering_time_threshold_shift > 0)
    {
...
/**
 * DetectAndRemoveLostPackets + OnPacketsLost 探测丢包+处理丢包
 */
void xqc_send_ctl_detect_lost(xqc_send_ctl_t *send_ctl, xqc_send_queue_t *send_queue, xqc_pkt_num_space_t pns, xqc_usec_t now)
{
    ...
    /* 测试loss_list的send部分 added by jndu*/
    int flag = 0;
    xqc_list_head_t *drop_pos = NULL, *drop_next = NULL;
    xqc_calc_loss_list_node_t *drop = NULL;
    xqc_list_for_each_safe(drop_pos, drop_next, &send_ctl->loss_list->list_head)
    {
        drop = xqc_list_entry(drop_pos, xqc_calc_loss_list_node_t, list_head);
    }
    ...
    /* Minimum time of kGranularity before packets are deemed lost. 在数据包被视为丢失之前，kGranularity的最短时间。*/
    loss_delay = xqc_max(loss_delay, XQC_kGranularity * 1000);

    /* Packets sent before this time are deemed lost. 在此时间之前发送的数据包被视为丢失。*/
    xqc_usec_t lost_send_time = now - loss_delay;

    /* Packets with packet numbers before this are deemed lost. 在此包号之前的包被视为丢失*/
    ...
    /* TODO: 在该函数中检测当次ack received丢包的包号范围并且计入链表中*/
    ...
    xqc_list_for_each_safe(pos, next, &send_queue->sndq_unacked_packets[pns])
    {
        ...
        /* If this packet is not lost, so is the next packet 如果这个数据包没有丢失，下一个数据包也没有丢失 */
        /* 如果该数据包未确认，则后续的数据包都不会被确认，跳出循环。
        因为detect_loss前已经经过ack确认，因此如果当前unack queue中仍未被确认,那么表示此包及其之后的包仅仅是未确认而不是丢失 */
        if (po->po_pkt.pkt_num > send_ctl->ctl_largest_acked[pns])
        {
            break;
        }

        /* Mark packet as lost, or set time when it should be marked. 将数据包标记为丢失，或设置应该标记的时间 */
        if (po->po_sent_time <= lost_send_time || (lost_pn != XQC_MAX_UINT64_VALUE && po->po_pkt.pkt_num <= lost_pn))
        {
            // 标记为丢失
            if (po->po_flag & XQC_POF_IN_FLIGHT)
            {
                // 更新飞行中的数据包数，并处理丢失的数据报
                xqc_send_ctl_decrease_inflight(conn, po);
                ...
                // 处理需要重传或由应用请求重传的数据包
                if (XQC_NEED_REPAIR(po->po_frame_types) || repair_dgram == XQC_DGRAM_RETX_ASKED_BY_APP)
                {
                    xqc_send_queue_copy_to_lost(po, send_queue);
                    // po需要重传 added by qnwang for repkt
                }
                else
                {
                    // 处理不需要重传的数据包
                    ...
                    {
                        /* remove the packet that does not need retransmission 移除不需要重传的数据包*/
                ...
                // 增加丢失数据包计数
                lost_n++;
                if (po->po_pkt.pkt_pns == XQC_PNS_APP_DATA)
                {
                    xqc_extra_sample_loss_list_update(send_ctl, po->po_pkt.pkt_num, XQC_TRUE);
                }
            ...
            /* remember largest_loss for OnPacketsLost 记录最大的丢失数据包*/
            if (largest_lost == NULL || (po->po_pkt.pkt_num > largest_lost->po_pkt.pkt_num))
            {
                largest_lost = po;
            }
        }
    ...
    /* update statistic 更新丢包数量*/
    send_ctl->ctl_lost_pkts_number += lost_n;
    send_ctl->sampler.loss = lost_n;

    if (pns == XQC_PNS_APP_DATA)
    {
        xqc_extra_sample_loss_list_update(send_ctl, send_ctl->ctl_largest_acked[pns] + 1, XQC_FALSE);
    }
    ...
    if (largest_lost)
    {
         ...
         * 如果最后一个丢失的数据包已经过了上一个恢复时期的末尾，则启动新的拥塞时期。
        ...
        /* Collapse congestion window if persistent congestion 如果持续拥塞，则折叠拥塞窗口 */
        if (send_ctl->ctl_cong_callback->xqc_cong_ctl_reset_cwnd && xqc_send_ctl_in_persistent_congestion(send_ctl, largest_lost, now))
        {
            /* For loss-based CCs, it means we are gonna slow start again. 对于基于损失的CC，这意味着我们将再次缓慢起步 */
            send_ctl->ctl_max_bytes_in_flight = 0;
            /* we reset BBR's cwnd here 重启bbr窗口 */
            ...
            if (conn->conn_settings.enable_CCA_switching)
            {
                ...
                if (future_cong_ctrl)
                {
                    if (future_cong_ctrl->xqc_cong_ctl_reset_cwnd)
                    {
                        future_cong_ctrl->xqc_cong_ctl_reset_cwnd(send_ctl->ctl_cong_stack[send_ctl->ctl_switch_ctx->ctx_future_state]);
                    }
                }
            }
        }
...
/**
 * InPersistentCongestion 判断是否持续拥塞
 */
xqc_bool_t
xqc_send_ctl_in_persistent_congestion(xqc_send_ctl_t *send_ctl, xqc_packet_out_t *largest_lost, xqc_usec_t now)
{
    ...
    {
        xqc_usec_t duration = (send_ctl->ctl_srtt + xqc_max(send_ctl->ctl_rttvar << 2, XQC_kGranularity * 1000) + send_ctl->ctl_conn->remote_settings.max_ack_delay * 1000) * XQC_kPersistentCongestionThreshold;
...
void xqc_send_ctl_congestion_event(xqc_send_ctl_t *send_ctl, xqc_usec_t sent_time)
{
    ...
    if (send_ctl->ctl_conn->conn_settings.enable_CCA_switching)
    {
        ...
        if (future_cong_ctrl)
        {
            if (future_cong_ctrl->xqc_cong_ctl_on_lost)
            {
                future_cong_ctrl->xqc_cong_ctl_on_lost(send_ctl->ctl_cong_stack[send_ctl->ctl_switch_ctx->ctx_future_state], sent_time);
            }
        }
    }
}
...
// 一个拥塞算法回调
void xqc_send_ctl_cc_on_ack(xqc_send_ctl_t *send_ctl, xqc_packet_out_t *acked_packet,
                            xqc_usec_t now)
{
    ...
    if (send_ctl->ctl_conn->conn_settings.enable_CCA_switching)
    {
        ...
        if (future_cong_ctrl)
        {
            if (future_cong_ctrl->xqc_cong_ctl_on_ack)
            {
                future_cong_ctrl->xqc_cong_ctl_on_ack(send_ctl->ctl_cong_stack[send_ctl->ctl_switch_ctx->ctx_future_state], acked_packet, now);
            }
        }
    }
...
// 用于处理 Path MTU Discovery（PMTUD）的 ping 数据包被确认的情况
void xqc_send_ctl_on_pmtud_ping_acked(xqc_send_ctl_t *send_ctl,
                                      xqc_packet_out_t *po)
{
    xqc_connection_t *conn = send_ctl->ctl_conn;
...
/**
 * OnPacketAcked 数据包确认之后
 */
void xqc_send_ctl_on_packet_acked(xqc_send_ctl_t *send_ctl,
                                  xqc_packet_out_t *acked_packet, xqc_usec_t now, int do_cc)
{
    ...
    if (packet_out->po_flag & XQC_POF_IN_FLIGHT)
    {
        ...
        {
            if (conn->app_proto_cbs.conn_cbs.conn_ping_acked && (packet_out->po_flag & XQC_POF_NOTIFY))
            {
...
// 获取 PTO（Probe Timeout）的超时时间和数据包编号空间
xqc_usec_t
xqc_send_ctl_get_pto_time_and_space(xqc_send_ctl_t *send_ctl, xqc_usec_t now, xqc_pkt_num_space_t *pns_ret)
{
    ...
    /* get pto duration */
    xqc_usec_t duration = (send_ctl->ctl_srtt + xqc_max(4 * send_ctl->ctl_rttvar, XQC_kGranularity * 1000)) * backoff;
...
/**
 * SetLossDetectionTimer 设置丢包探测计时器
 */
void xqc_send_ctl_set_loss_detection_timer(xqc_send_ctl_t *send_ctl)
{
    ...
    if (loss_time != 0)
    {
        xqc_log(conn->log, XQC_LOG_DEBUG, "|xqc_timer_set|earliest loss time|XQC_TIMER_LOSS_DETECTION|"
                                          "conn:%p|path:%ui|pns:%d|expire:%ui|now:%ui|interval:%ui|",
                conn, path->path_id, pns, loss_time, now, interval);
    ...
    if (0 == send_ctl->ctl_bytes_ack_eliciting_inflight[XQC_PNS_INIT] && 0 == send_ctl->ctl_bytes_ack_eliciting_inflight[XQC_PNS_HSK] && 0 == send_ctl->ctl_bytes_ack_eliciting_inflight[XQC_PNS_APP_DATA] && xqc_conn_peer_complete_address_validation(conn))
    {
...
/**
 * GetLossTimeAndSpace 获得丢包时间和包编号空间
 */
xqc_usec_t
xqc_send_ctl_get_earliest_loss_time(xqc_send_ctl_t *send_ctl, xqc_pkt_num_space_t *pns_ret)
{
    ...
    *pns_ret = XQC_PNS_INIT;
    ...
    {
        if (send_ctl->ctl_loss_time[pns] != 0 && (time == 0 || send_ctl->ctl_loss_time[pns] < time))
        {
...
// 检查是否存在反放大攻击。反放大攻击是指在连接的早期阶段发送大量数据以引发响应，从而占用对端资源
xqc_bool_t
xqc_send_ctl_check_anti_amplification(xqc_send_ctl_t *send_ctl, size_t send_bytes)
{
    ...
    if (check)
    {
        limit = (send_ctl->ctl_bytes_send + send_bytes >= conn->conn_settings.anti_amplification_limit * send_ctl->ctl_bytes_recv);
    }
...
// 检查在给定的数据包号空间（Packet Number Space，pns）中是否接收到了 ACK
xqc_bool_t
